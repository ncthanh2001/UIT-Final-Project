    ...
    def analyze_suppliers(self, company):
        ...
        for supplier, orders in supplier_groups.items():
            try:
                ...
                total_value = sum(
                    float(order.get("qty", 0)) * float(order.get("rate", 0))
                    for order in orders
                )
        ...
        try:
            ...
            for result in results:
                forecasts = frappe.get_all(
                    ...
                    filters={"company": company, "supplier": result["supplier"]},
                    fields=["name"],
                )
                ...
                for forecast in forecasts:
                    try:
                        ...
                        frappe.log_error(
                            f"Failed to update forecast {forecast.name}: {str(e)}"
                        )
    ...
    def predict_item_price(self, item_code, supplier, company, qty=1):
        ...
        try:
            ...
            historical_prices = frappe.db.sql(
                ...
                LIMIT 10
            """,
                (item_code, supplier, company),
                as_dict=True,
            )

            if historical_prices:
                # Simple price prediction - average of recent prices
                avg_price = sum(p["rate"] for p in historical_prices) / len(
                    historical_prices
                )
    ...
    try:
        ...
        if not raw_data:
            ...
            po_count = frappe.db.count(
                "Purchase Order", {"company": company, "docstatus": 1}
            )
    ...
    try:
        ...
        for company in companies:
            try:
                ...
                frappe.logger().info(
                    f"Daily ML analysis for {company.name}: {result.get('message', 'Completed')}"
                )

            except Exception as e:
                frappe.log_error(
                    f"Daily ML analysis failed for {company.name}: {str(e)}"
                )
    ...
    try:
        ...
        for company in companies:
            try:
                ...
                frappe.logger().info(
                    f"Weekly segmentation for {company.name}: {len(results)} suppliers segmented"
                )

            except Exception as e:
                frappe.log_error(
                    f"Weekly segmentation failed for {company.name}: {str(e)}"
                )
            ...
            try:
                ...
                try:
                    ...
                    if supplier_meta.has_field("supplier_segment"):
                        frappe.db.set_value(
                            "Supplier", supplier_name, "supplier_segment", segment
                        )

                    if supplier_meta.has_field("risk_score"):
                        frappe.db.set_value(
                            "Supplier", supplier_name, "risk_score", risk_score
                        )

                    if supplier_meta.has_field("deal_score"):
                        frappe.db.set_value(
                            "Supplier", supplier_name, "deal_score", score
                        )
                    ...
                    frappe.db.set_value(
                        "Supplier", supplier_name, "modified", frappe.utils.now()
                    )
                    ...
                    frappe.log_error(
                        f"Field update failed for {supplier_name}: {str(field_error)}"
                    )
                    continue

            except Exception as e:
                frappe.log_error(
                    f"Failed to update supplier {result.get('supplier', 'Unknown')}: {str(e)}"
                )
                continue
...
def check_company_purchase_data(company):
    ...
    try:
        ...
        return {"status": "error", "message": str(e), "has_data": False}
    ...
    try:
        ...
        if data:
            ...
            for res in analysis_results:
                print(
                    f"  • {res['supplier']}: Score {res['score']} ({res['recommendation']})"
                )
            ...
            if main_result.get("status") == "success":
                ...
                result["message"] = (
                    f"❌ Main function still failed: {main_result.get('message')}"
                )
