...
def on_stock_ledger_entry_submit(doc, method):
    ...
    Updates APS AI Inventory Forecast records for affected items - THREAD SAFE
    """
    try:
        ...
        safe_forecast_update_from_stock_entry(
            doc.item_code, doc.warehouse, warehouse_company, doc.actual_qty
        )

    except Exception as e:
        # Use simple logging to avoid cascade errors
        frappe.logger().error(
            f"AI Forecast update failed on stock movement: {str(e)[:100]}"
        )
...
def safe_forecast_update_from_stock_entry(item_code, warehouse, company, qty_change):
    ...
    try:
        # Check if there's an APS AI Inventory Forecast for this item-warehouse-company combo
        forecast_name = frappe.db.exists(
            "APS AI Inventory Forecast",
        ...
        if forecast_name:
            ...
            frappe.db.sql(
                """
                UPDATE `tabAPS AI Inventory Forecast`
                SET current_stock = (
...
def on_item_after_insert(doc, method):
    ...
    Auto-creates APS AI Inventory Forecast records for stock items
    """
    try:
        ...
        if created_count > 0:
            frappe.logger().info(
                f"Created {created_count} AI Forecasts for new item: {doc.name}"
            )

    except Exception as e:
        # Use simple logging to avoid cascade errors
        frappe.logger().error(
            f"Failed to create forecasts for new item {doc.name}: {str(e)[:100]}"
        )


def create_forecasts_for_new_item(item_code):
    """Create APS AI Inventory Forecasts for a new item across all warehouses"""
    try:
        created_count = 0

        # Get all active warehouses
        warehouses = frappe.get_all("Warehouse", filters={}, fields=["name", "company"])
            ...
            try:
                ...
                if not existing:
                    ...
                    forecast = frappe.get_doc(
                        {
                            "doctype": "APS AI Inventory Forecast",
                            "item_code": item_code,
                            "warehouse": warehouse.name,
                            "company": warehouse.company,
                ...
                # Use simple logging to avoid cascade errors
                frappe.logger().error(
                    f"Failed to create forecast for {item_code} in {warehouse.name}: {str(e)[:100]}"
                )
                continue
        ...
        frappe.logger().error(
            f"Failed to create forecasts for item {item_code}: {str(e)[:100]}"
        )
        return 0
...
def queue_item_forecast_creation(item_code):
    ...
    try:
        ...
        frappe.logger().error(
            f"Failed to queue forecast creation for {item_code}: {str(e)[:100]}"
        )
...
def on_item_on_update(doc, method):
    ...
    Updates related APS AI Inventory Forecasts if item properties change
    """
    try:
        ...
        frappe.logger().error(
            f"Failed to update forecasts for item {doc.name}: {str(e)[:100]}"
        )
...
def disable_item_forecasts(item_code):
    ...
    try:
        ...
        frappe.logger().error(
            f"Failed to disable forecasts for {item_code}: {str(e)[:100]}"
        )
...
def enable_item_forecasts(item_code):
    ...
    try:
        ...
        frappe.logger().error(
            f"Failed to enable forecasts for {item_code}: {str(e)[:100]}"
        )
...
def on_warehouse_after_insert(doc, method):
    ...
    Creates APS AI Inventory Forecasts for all existing stock items
    """
    try:
        if doc.disabled:
            return
        ...
        if created_count > 0:
            frappe.logger().info(
                f"Created {created_count} AI Forecasts for new warehouse: {doc.name}"
            )

    except Exception as e:
        frappe.logger().error(
            f"Failed to create forecasts for new warehouse {doc.name}: {str(e)[:100]}"
        )


def create_forecasts_for_new_warehouse(warehouse, company):
    """Create APS AI Inventory Forecasts for all items in a new warehouse"""
    try:
        created_count = 0
        ...
        for item in items:
            try:
                ...
                frappe.logger().error(
                    f"Failed to create forecast for {item.name} in {warehouse}: {str(e)[:100]}"
                )
                continue
        ...
        frappe.logger().error(
            f"Failed to create forecasts for warehouse {warehouse}: {str(e)[:100]}"
        )
        return 0
...
def queue_warehouse_forecast_creation(warehouse, company):
    ...
    try:
        frappe.enqueue(
            "uit_aps.hooks_handlers.create_forecasts_for_new_warehouse",
            warehouse=warehouse,
            company=company,
            queue="long",
            timeout=1200,
        ...
        frappe.logger().error(
            f"Failed to queue forecast creation for warehouse {warehouse}: {str(e)[:100]}"
        )
...
def handle_bulk_item_import():
    ...
    try:
        ...
        items_without_forecasts = frappe.db.sql(
            ...
            AND i.disabled = 0
            AND NOT EXISTS (
                SELECT 1 FROM `tabAPS AI Inventory Forecast` aif 
        ...
        for i in range(0, len(items_without_forecasts), batch_size):
            ...
            for item in batch:
                try:
                    ...
                    frappe.logger().error(
                        f"Bulk import forecast creation failed for {item.name}: {str(e)[:100]}"
                    )
                    continue
...
def bulk_create_forecasts_for_existing_items(company=None):
    """Bulk create APS AI Inventory Forecast records - THREAD SAFE"""
    try:
        # Set bulk operation flag
        frappe.flags.in_bulk_operation = True
        ...
        #     as_dict=True,
        # )
        # mỗi lần tạo sẽ ghi nhận lịch sử khác nhau
        items_without_forecasts = frappe.db.sql(
            f"""
            SELECT DISTINCT 
                i.name as item_code, 
        ...
        # tạo lịch sử  APS AI Invetory Forecast History
        ai_inventory_settings = frappe.get_single("AI Inventory Settings")
        lead_time_days = ai_inventory_settings.default_lead_time_days
        forecast_period_days = ai_inventory_settings.forecast_period_days

        history = frappe.new_doc("APS AI Inventory Forecast History")
        history.ml_model = ai_inventory_settings.ml_model
        history.forecast_period_days = forecast_period_days
        history.default_lead_time_days = lead_time_days
        ...
        for item in items_without_forecasts:
            try:
                ...
                frappe.logger().error(
                    f"Failed to create forecast for {item.item_code}: {str(e)[:100]}"
                )
                print(e)
        ...
        company_summary = ", ".join(
            [f"{comp}: {count}" for comp, count in company_counts.items()]
        )
    ...
    try:
        ...
        for item in items_with_stock:
            try:
                ai_inventory_settings = frappe.get_single("AI Inventory Settings")
                forecast_period_days = ai_inventory_settings.forecast_period_days
                lead_time_days = ai_inventory_settings.lead_time_days
        ...
        frappe.logger().error(
            f"Auto-create for items with stock failed: {str(e)[:100]}"
        )
        return {"status": "error", "message": str(e)}
    finally:
        frappe.flags.in_bulk_operation = False
...
def trigger_immediate_forecast_update(item_code, warehouse):
    ...
    try:
        # Find the forecast
        forecast_name = frappe.db.get_value(
            "APS AI Inventory Forecast",
        ...
        if forecast_name:
            ...
            return {"status": "error", "message": "No forecast record found"}
...
def process_forecast_update_queue():
    ...
    try:
            ...
            for update in batch:
                try:
                    forecast_doc = frappe.get_doc(
                        "APS AI Inventory Forecast", update["forecast_id"]
                    )
                    ...
                    frappe.logger().error(
                        f"Batch forecast update failed for {update['forecast_id']}: {str(e)[:100]}"
                    )
        ...
        frappe.logger().error(
            f"Forecast update queue processing failed: {str(e)[:100]}"
        )
    ...
    try:
        if hasattr(doc, "_doc_before_save"):
            ...
            if abs(new_qty - old_qty) > 10:
                frappe.logger().info(
                    f"Significant stock change for {doc.item_code}: {old_qty} -> {new_qty}"
                )
    except Exception as e:
